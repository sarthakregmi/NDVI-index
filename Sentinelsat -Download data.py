...
def datadownload():
    if call.sentinel2box.isChecked() == True:
        ...
        products = api.query(
            footprint,
            date=(start_date, date(2022, 10, 21)),
...
def openFileNameDialog():
    ...
    Data_path = QFileDialog.getOpenFileName(
        None, "Open File", "", "Shape Files (*.shp));;geojson files (*.geojson)"
    )
...
def getImage():
    ...
    img_path = granule_path + "/" + os.listdir(granule_path)[0] + "/IMG_DATA/R10m"
    red_path = img_path + "/" + os.listdir(img_path)[3]
    nir_path = img_path + "/" + os.listdir(img_path)[4]
...
def get_NDVI():
    ...
    plt.axis("off")
    plt.show()
    plt.imsave("output/ndvi_cmap1.png", ndvi, cmap=plt.cm.summer)
